...
export const fetchIconSymbolAndNameHistorical = ({
  ...
  const reservePatch = fetchIconSymbolAndName({
    underlyingAsset,
    symbol,
    name,
  });
  ...
  return data.map((transaction: TransactionHistoryItemUnion) => {
    ...
    if (hasCollateralReserve(transaction) && transaction.collateralAmount) {
      ...
      if (csv) {
        ...
        newTransaction.collateralReserve = `"${jsonString.replace(
          /"/g,
          '""'
        )}"`;
      }
    ...
    if (hasSwapBorrowRate(transaction)) {
      ...
      newTransaction.variableBorrowRate =
        formatUnits(transaction.variableBorrowRate, 25) + " %";
      newTransaction.stableBorrowRate =
        formatUnits(transaction.stableBorrowRate, 25) + " %";
    }
