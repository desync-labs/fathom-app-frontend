...
import {
  ...
  setOpenModal,
  updateBlockNumber,
} from "apps/dex/state/application/actions";
...
describe("application reducer", () => {
  ...
  describe("updateBlockNumber", () => {
    it("updates block number", () => {
      store.dispatch(
        updateBlockNumber({ chainId: ChainId.XDC, blockNumber: 4 })
      );
      expect(store.getState().blockNumber[ChainId.XDC]).toEqual(4);
    });
    it("no op if late", () => {
      store.dispatch(
        updateBlockNumber({ chainId: ChainId.XDC, blockNumber: 2 })
      );
      expect(store.getState().blockNumber[ChainId.XDC]).toEqual(3);
    });
    it("works with non-set chains", () => {
      store.dispatch(
        updateBlockNumber({ chainId: ChainId.AXDC, blockNumber: 2 })
      );
