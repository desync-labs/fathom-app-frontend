...
export class XdcInjectedConnector extends AbstractConnector {
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId;
    try {
      chainId = await (window.xdc.send as Send)("eth_chainId").then(
        parseSendReturn
      );
    } catch {
      warning(
    ...
    if (!chainId) {
      try {
        chainId = await (window.xdc?.send as Send)("net_version").then(
          parseSendReturn
        );
      } catch {
        warning(
    ...
    if (!chainId) {
      try {
        chainId = parseSendReturn(
          (window.xdc.send as SendOld)({ method: "net_version" })
        );
      } catch {
        warning(
          false,
    ...
    if (!chainId) {
      if ((window.xdc as any).isDapper) {
        chainId = parseSendReturn(
          (window.xdc as any).cachedResults.net_version
        );
      } else {
        chainId =
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account;
    try {
      account = await (window.xdc.send as Send)("eth_accounts").then(
        (sendReturn) => parseSendReturn(sendReturn)[0]
      );
    } catch {
    ...
    if (!account) {
      account = parseSendReturn(
        (window.xdc.send as SendOld)({ method: "eth_accounts" })
      )[0];
    }
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.xdc.send as Send)("eth_accounts").then(
        (sendReturn) => {
