          ...
          <StableSwapPaper>
            <StableSwapInfoContainer>
              <StableSwapInfoWrapper>
                ...
                <InfoValue>{
                  formatPercentage(
                    BigNumber(feesRewardsForFXD).dividedBy(10 ** 18).toNumber()
                  )} FXD</InfoValue>
              </StableSwapInfoWrapper>
              <StableSwapInfoWrapper>
            ...
            { feesRewardsForFXD > 0 || feesRewardsForUSDTx > 0 ? <SwapButton
              isLoading={claimRewardsInProgress}
              disabled={claimRewardsInProgress}
              onClick={claimFeesRewards}
            >
              {claimRewardsInProgress ? <CircularProgress size={30} /> : "Claim Fees Rewards"}
            </SwapButton> : null }
            ...
            <StableSwapInfoContainer>
              <StableSwapInfoWrapper>
                ...
                <InfoValue>{
                  formatPercentage(
                    BigNumber(claimedFXDFeeReward).dividedBy(10 ** 18).toNumber()
                  )} FXD</InfoValue>
              </StableSwapInfoWrapper>
              <StableSwapInfoWrapper>
            ...
            { claimedFXDFeeReward > 0 || claimedUSDTFeeReward > 0 ? <SwapButton
              isLoading={withdrawClaimedFeesInProgress}
              disabled={withdrawClaimedFeesInProgress}
              onClick={withdrawClaimedFees}
            >
              {withdrawClaimedFeesInProgress ? <CircularProgress size={30} /> : "Withdraw Claimed Fees"}
            </SwapButton> : null }
