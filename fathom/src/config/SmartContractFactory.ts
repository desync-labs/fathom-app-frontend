...
import StableSwapModuleWrapper from "config/ABI/StableSwapModuleWrapper.json";
...
export class SmartContractFactory {
  public static Addresses(chainId: number) {
    try {
      const environment = (process.env.REACT_APP_ENV as string) ?? "dev";
      let address: any;
  ...
  public static StableSwapModuleWrapper(chainId: number) {
    return {
      ...
      address: SmartContractFactory.Addresses(chainId).stableSwapModuleWrapper,
    };
  }
