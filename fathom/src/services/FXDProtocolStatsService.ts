...
export default class FXDProtocolStatsService implements IFXDProtocolStatsService{
    chainId = Constants.DEFAULT_CHAINID;
    
    async fetchProtocolStats(): Promise<IFXDProtocolStats> {
        try{
            console.log(`Fetching Protocol Stats.`)
            let protocolStatContract = Web3Utils.getContractInstance(SmartContractFactory.FathomStats(this.chainId),this.chainId)
            let response = await protocolStatContract.methods.getFathomInfo().call();
  
            const { 0: fathomSupplyCap, 1: totalValueLocked, 2: fxdPriceFromDex, 3: liquidationRatio, 4: closeFactor } = response;
