...
async approveStablecoin(address:string, transactionStore:ActiveWeb3Transactions): Promise<void>{
    try{
        ...
        await fathomStableCoin.methods.approve(SmartContractFactory.StableSwapModule(this.chainId).address,  Constants.MAX_UINT256)
        .send({from:address})
        .on('transactionHash', (hash:any) => {
            transactionStore.addTransaction({
                    hash:hash, 
...
async approveUsdt(address:string, tokenIn:number, transactionStore:ActiveWeb3Transactions): Promise<void>{
    try{
        ...
        await USDT.methods.approve(SmartContractFactory.AuthtokenAdapter(this.chainId).address,  Constants.MAX_UINT256)
        .send({from:address})
        .on('transactionHash', (hash:any) => {
                                    transactionStore.addTransaction({
                                            hash:hash, 
...
async approvalStatusStablecoin(address:string, tokenIn:number): Promise<Boolean>{
    try{
      ...
      let allowance = await fathomStableCoin.methods.allowance(address, SmartContractFactory.StableSwapModule(this.chainId).address).call()
...
async approvalStatusUsdt(address:string, tokenIn:number): Promise<Boolean>{
    try{
      const USDT = Web3Utils.getContractInstance(
        ...
        let allowance = await USDT.methods.allowance(address, SmartContractFactory.AuthtokenAdapter(this.chainId).address).call()
