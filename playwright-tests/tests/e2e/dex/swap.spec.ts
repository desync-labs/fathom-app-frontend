...
test.describe("Fathom App Test Suite: DEX Swap", () => {
  test(
    qase(43, "Swapping XDC with xUSDT is successful @smoke"),
    async ({ dexPage }) => {
  ...
  test(
    qase(44, "Wrapping XDC to WXDC is successful @smoke"),
    async ({ dexPage }) => {
      await dexPage.navigate();
      await dexPage.connectWallet(WalletConnectOptions.Metamask);
      await dexPage.validateConnectedWalletAddress();
  ...
  test(
    qase(45, "Unwrapping WXDC to XDC is successful @smoke"),
    async ({ dexPage }) => {
      await dexPage.navigate();
      await dexPage.connectWallet(WalletConnectOptions.Metamask);
      await dexPage.validateConnectedWalletAddress();
  ...
  test(
    qase(46, "Swap: 'Insufficient balance' is correctly displayed"),
    async ({ dexPage }) => {
  ...
  test(
    qase(47, "Wrap: 'Insufficient balance' is correctly displayed"),
    async ({ dexPage }) => {
  ...
  test(
    qase(
      48,
      "Wallet not connected state layout is correct, dex connect wallet functionality is successful"
    ),
    async ({ dexPage }) => {
      const tokenFrom = xdcData;
      const tokenTo = xUsdtData;
      ...
      await expect(dexPage.swapConnectWalletButton).toHaveText(
        "Connect Wallet"
      );
      await dexPage.selectFromToken({ tokenData: tokenFrom });
      await expect(dexPage.swapConnectWalletButton).toBeVisible();
  ...
  test(
    qase(49, "Swap: 'Enter an amount' is correctly displayed"),
    async ({ dexPage }) => {
      const tokenFrom = xdcData;
      const tokenTo = xUsdtData;
      await dexPage.navigate();
  ...
  test(
    qase(50, "Wrap: 'Enter an amount' is correctly displayed"),
    async ({ dexPage }) => {
      const tokenFrom = xdcData;
      const tokenTo = wxdcData;
      await dexPage.navigate();
