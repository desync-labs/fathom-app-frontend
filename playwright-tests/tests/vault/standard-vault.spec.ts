...
test.describe("Fathom App Test Suite: Vault Operations - Lending & Liquidation Vault", () => {
  ...
  test(
    qase(
      22,
      "Vault List Item Page - Manage Vault: Depositing 50 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      23,
      "Vault List Item Page - Manage Vault: Partially withdrawing 50 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      24,
      "Vault List Item Page - Deposit: Depositing first 0.5 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      25,
      "Vault List Item Page: Wallet not connected state layout is correct, vault connect wallet functionality is successful"
    ),
    async ({ vaultPage }) => {
      await vaultPage.navigate();
      ...
      await expect
        .soft(
          vaultPage.getDepositButtonRowLocatorById(
  ...
  test(
    qase(
      26,
      "Vault Detail Page - Manage Vault: Depositing 100.5 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      27,
      "Vault Detail Page - Manage Vault: Partially withdrawing 100.5 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      28,
      "Vault Detail Page - Deposit: Depositing first 0.85 FXD is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      29,
      "Vault Detail Page - Manage Vault: Fully withdrawing all FXD is successful @smoke"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      30,
      "Vault Detail Page: Wallet not connected state layout is correct, vault connect wallet functionality is successful"
    ),
    async ({ vaultPage }) => {
      await vaultPage.navigate();
      await vaultPage.openVaultDetails(lendingLiquidationVaultData.id);
      ...
      await expect(vaultPage.btnConnectWallet).not.toBeVisible({
        timeout: 5000,
      });
      const balanceValueConnected =
        await vaultPage.getBalanceVaultDetailValue();
