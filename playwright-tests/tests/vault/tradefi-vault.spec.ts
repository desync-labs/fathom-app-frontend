...
test.describe("Fathom App Test Suite: Vault Operations - TradeFintech Vault", () => {
  ...
  test(
    qase(
      21,
      "Deposit Period - Depositing more than or equal to the minimum required amount is successful @smoke"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      31,
      "Deposit Period - Depositing any amount when minimum required amount is already deposited is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      32,
      "Deposit Period - Trying to deposit less than the required amount is not possible and returns corresponding error"
    ),
    async ({ vaultPage }) => {
      ...
      await vaultPage.enterDepositAmountVaultListItemDepositModal(
        depositAmount
      );
      await expect
  ...
  test(
    qase(
      33,
      "Deposit Period - Withdrawing amount that will result in more than or equal to required remaining amount is successful"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      34,
      "Deposit Period - Trying to withdraw amount that would result in less than required remaining amount is not possible and returns corresponding error"
    ),
    async ({ vaultPage }) => {
      const depositAmount = 10100;
      ...
      await vaultPage.enterWithdrawAmountVaultListItemManageModal(
        withdrawAmount
      );
      await vaultPage.page.waitForTimeout(2000);
  ...
  test(
    qase(35, "Deposit Period - Fully withdrawing all funds is successful"),
    async ({ vaultPage }) => {
      const depositAmount = 10000;
      await vaultPage.depositFirstTimeVaultListItem({
        id,
  ...
  test(
    qase(
      36,
      "Deposit Period - Non-KYC verified user cannot deposit and a proper warning message is displayed"
    ),
    async ({ vaultPage }) => {
  ...
  test(
    qase(
      37,
      "Lock Period - Layout is correct, deposit, withdraw and withdraw all funds are not available @smoke"
    ),
    async ({ vaultPage }) => {
